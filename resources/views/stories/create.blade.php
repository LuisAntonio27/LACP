                    ...
                    <form action="{{ route('stories.store') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="type" id="type" class="form-control @error('type') is-invalid @enderror">
                                ...
                                <option value="short" {{'short' == old('type', '') ? 'selected' : ''}}>Corta</option>
                                <option value="long" {{'long' == old('type', '') ? 'selected' : ''}}>Larga</option>
                            </select>
                            @error('type')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        ...
                        <fieldset class="form-group">
                            ...
                            <div class="form-check">
                                <input type="radio" class="form-check-input" name="status" id="status1" value="1" {{'1' == old('status', '') ? 'checked' : ''}}>
                                <label for="status1" class="form-check-label">S&iacute;</label>
                            </div>
                            <div class="form-check">
                                <input type="radio" class="form-check-input" name="status" id="status2" value="0" {{'0' == old('status', '') ? 'checked' : ''}}>
